...
def main():
    ...
    # 2. Scan
    run_command(
        f"terrain scan {repo_path} --db-path {db_path} --clean",
        "2. 扫描代码仓库"
    )

    run_command(
        f"terrain scan {repo_path} \\\n"
        f"  --db-path {db_path} \\\n"
    ...
    # 4. Stats
    run_command(
        f"terrain stats --db-path {db_path}",
        "6. 查看统计信息"
    )

    # 5. Export
    run_command(
        f"terrain export {repo_path} \\\n"
    ...
    print("运行: terrain --version")
    result = subprocess.run(
        [sys.executable, "-m", "terrain.cli", "--version"],
        capture_output=True,
        text=True
    )
    print(result.stdout or result.stderr)

    print("运行: terrain stats --help")
    result = subprocess.run(
        [sys.executable, "-m", "terrain.cli", "stats", "--help"],
        capture_output=True,
        text=True
    )
    print(result.stdout)
    ...
    print()
    print("快速参考:")
    print("  terrain scan <repo> --db-path <path>  # 扫描代码")
    print("  terrain query '<cypher>'               # 查询")
    print("  terrain stats                          # 统计")
    print("  terrain export <repo> -o <file>        # 导出")
    print()
    print("详细文档: CLI.md")
