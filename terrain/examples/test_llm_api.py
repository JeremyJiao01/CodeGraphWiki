...
Reads ``~/.terrain/.env`` (written by ``npx terrain --setup``)
...
Usage:
    python -m terrain.examples.test_llm_api
"""
...
# 1. Load .env
# ---------------------------------------------------------------------------
_NPX_ENV = Path.home() / ".terrain" / ".env"
...
for env_path in (_NPX_ENV, _LOCAL_ENV):
    ...
    logger.warning(
        ...
        f"  Expected: {_NPX_ENV}\n"
        "  Run `npx terrain --setup` to create one."
    )

# Import after env is loaded so auto-detection picks up the keys
from terrain.rag.llm_backend import create_llm_backend  # noqa: E402
...
def test_llm_connection() -> bool:
    ...
    if not backend.available:
        logger.error(
            ...
            "Run `npx terrain --setup` to configure interactively."
        )
        return False
...
def test_embedding_connection() -> bool:
    ...
    try:
        from terrain.embeddings.qwen3_embedder import create_embedder
