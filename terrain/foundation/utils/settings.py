"""Configuration loader for terrain.

All configuration is read exclusively from the workspace ``.env`` file
(``~/.terrain/.env`` by default, overridable via
``TERRAIN_WORKSPACE``).  No other files (local ``.env``, ``settings.json``,
...
def reload_env(workspace: Path | None = None) -> dict[str, list[str]]:
    ...
    Args:
        workspace: Workspace directory (default: ``~/.terrain``).
    ...
    # All config-managed keys.  TERRAIN_WORKSPACE is intentionally excluded —
