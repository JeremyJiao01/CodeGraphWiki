...
Usage:
    python3 -m terrain.commands_cli <command> [args...]
...
from terrain.foundation.services.git_service import GitChangeDetector as _GCD  # noqa: E402
...
def cmd_graph_build(args: argparse.Namespace, ws: Workspace) -> None:
    ...
    from terrain.entrypoints.mcp.pipeline import artifact_dir_for, build_graph, save_meta
...
def cmd_api_doc_gen(args: argparse.Namespace, ws: Workspace) -> None:
    ...
    from terrain.entrypoints.mcp.pipeline import generate_api_docs_step, save_meta
...
def cmd_wiki_gen(args: argparse.Namespace, ws: Workspace) -> None:
    from terrain.examples.generate_wiki import MAX_PAGES_COMPREHENSIVE, MAX_PAGES_CONCISE
    from terrain.entrypoints.mcp.pipeline import build_vector_index, run_wiki_generation, save_meta
...
def cmd_embed_gen(args: argparse.Namespace, ws: Workspace) -> None:
    from terrain.entrypoints.mcp.pipeline import build_vector_index, save_meta
