...
## 阶段 0.5：知识库闪查（检查是否已有追溯结果）

在执行追溯之前，检查知识库中是否已有该函数的调用链分析。
...
2. 尝试读取 `{artifact_dir}/kb/index.md`
   - 文件不存在 → 跳过，进入阶段 1
3. 在 `index.md` 中匹配目标函数名（`$ARGUMENTS`）——查找带 `[trace]` 标记的条目
   - 命中 → 读取对应的 wiki 文件，展示并询问用户：

> *"📚 找到了之前对 `<函数名>` 的调用链分析——"*
>
> [展示报告摘要：目标函数、直接调用者数、入口点数、调用路径数]
>
> *"你想：A) 直接查看已有报告？B) 重新追溯（如代码已更新）？"*

- 用户选 A → 展示已有 wiki 页面内容，跳到阶段 6 的建议下一步部分
- 用户选 B → 忽略已有结果，按正常流程执行，最终覆盖旧条目
...
3. 在 `{artifact_dir}/kb/index.md` 中追加或更新一行：
   ```
   - [trace] [调用链：{target_function}](../wiki/call-traces/{wiki_filename}) | {函数名}, {模块名}, 调用链, call chain, {直接调用者名1}, {直接调用者名2}, ...
   ```
   - `[trace]` 标记用于区分调用链报告和 `/ask` 的知识条目、`/research` 的研究报告
   - 关键词应覆盖：目标函数名、所在模块、主要调用者名（最多 5 个）
   - 如果同名条目已存在（同一目标函数），覆盖该行
